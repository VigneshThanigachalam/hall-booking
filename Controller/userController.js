...
export const forgetPasswordtoken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const data = {
      to: email,
      subject: "Hall Booking App - Reset Password Link",
...
export const bookings = asyncHandler(async (req, res) => {
  const { name } = req.user;
  const find_bookings = await room.find({ "bookingList.name": name });

  res.json({
    no_of_bookings: find_bookings.length,
    bookings_list: find_bookings
  })
})